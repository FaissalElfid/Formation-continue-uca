    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                $this->data['departement_id'] = $post["departement_id"];
...
//        echo 'console.log("'.$this.data.'");';
    ...
    // showing disable authentication staf list
    public function disable_authentication()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $this->data['stafflist'] = $this->employee_model->getStaffList(1, $role, 0);
        }
    ...
    public function csv_import()
    {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            if ($csv_array) {
                $columnHeaders = array('Name','Gender','DateOfBirth','MobileNo','PresentAddress','Email','Password');
                $csvData = array();
                    ...
                    if (count($checkCSV) <= 0) {
                        if (filter_var($row['Email'], FILTER_VALIDATE_EMAIL)) {
                            ...
                            if ($query->num_rows() > 0) {
                                ...
                                // save all employee information in the database
                                $this->employee_model->csvImport($row, $branchID, $userRole);
                                $i++;
                            }
                        } else {
                ...
                if ($err_msg != null) {
                    $msgRes = $i . ' Staffs Have Been Successfully Added. <br>';
                    $msgRes .= $err_msg;
                    echo json_encode(array('status' => 'errlist', 'errMsg' => $msgRes));
                    exit();
                }
                if ($i > 0) {
                    set_alert('success', $i . ' Staffs Have Been Successfully Added');
                }
            } else {
                set_alert('error', translate('invalid_csv_file'));
            }
